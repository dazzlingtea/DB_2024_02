 ...
 */
SELECT trunc(sysdate - DATE '2010-12-31') AS "태어난 지 (일수)"
FROM dual;
...
SELECT sysdate AS "현재 날짜",
       sysdate - 1 AS "1일전",
       sysdate - 1/24 AS "1시간전",
       sysdate - 1/60/24 AS "1분전",
       sysdate - 1/60/60/24 AS "1초전"
FROM dual;
...
FROM employees
ORDER BY "근속일수" DESC, "사원번호"; -- SELECT 컬럼 2개니까 사원번호 1 근속일수 2
...
-- 문자 || 문자 (띄어쓰기 없음 -> 띄어쓰기 ' ' 도 ||로 연결)
SELECT first_name || last_name AS full_name,
	first_name || ' ' || last_name
FROM employees;

-- 숫자 || 문자
-- 숫자를 문자로 자동 형변환 한 후에 문자를 연결한다
SELECT salary || '원'
FROM employees;
...
SELECT 20 + 30 || '원' -- 50 || '원'
	--,'원' || 20 + 30  '원20' + 30 '원||20'이 먼저 (문자+숫자) invalid 오류
FROM dual;
...
-- 만약 해당 사원의 salary 컬럼의 값이 10000 보다 크기가 같으면 TRUE,
-- 아니라면 FALSE의 결과를 얻게 되고 그 값이 TURE가 나온 사원만 조회한다
SELECT *
FROM employees
WHERE salary >= 10000;
...
/*
 * 실습1. 이름(first_name)이 David인 사원의 이름, 성, 급여 조회하기
 */
...
-- 실행순서 : from > where > select > order by  (별명은 select에서 줘서 그 전 실행 땐 문제 생김)
SELECT department_id, salary
FROM employees
WHERE department_id = 80
...
-- where은 select 이전에 실행되므로 별칭을 사용할 수 없다
...
-- 현실세계에서는 표기가 가능하지만, 문법저긍로 틀려 에러 발생
--SELECT salary
--FROM EMPLOYEES
...
-- 1번과 2번 조건 둘 중에 한개라도 만족하는 사원을 조회하기
